...
Sends emails to warmest leads first (warmth_score >= 6).
...
def get_eligible_leads(leads, sent):
    ...
    if sent_today >= MAX_EMAILS_PER_DAY:
        print(f"Already sent {sent_today} emails today (limit: {MAX_EMAILS_PER_DAY}).")
        return []
...
def personalize(template, lead, sent_data=None):
    ...
    elif "has_pdf_documents" in signals:
        warmth_intro = "Ich habe gesehen, dass Sie PDF-Dokumente auf Ihrer Website anbieten — diese könnten automatisch erstellt und aktualisiert werden."
    elif "no_chatbot" in signals:
        warmth_intro = "Ich habe Ihre Website besucht und festgestellt, dass Sie noch keinen KI-Assistenten für Kundenanfragen einsetzen."
