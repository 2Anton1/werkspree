...
- **Rechtliche Freigabe durch Eigentümer erteilt** — E-Mail-Outreach ist aktiv
  (läuft bereits automatisch via Cron `df4d149e4f8f` täglich 10:00, Auto-Send
  ohne `--dry-run`, max 10 Mails/Tag, Score ≥ 6, nur deep-researched Leads,
  Absender „Finn Werksby" via Strato SMTP `werkspree@bki-de.de`).
- Lauf 03.09. 10:00: 7/8 Mails erfolgreich versendet (Follow-ups via Strato).
- **Fix:** `warm_outreach.py` filtert jetzt unbekannte TLDs. Die Encoding-Fehler-
  Adressen `elmcha@r-utneodegkwt-lmales.nivzrisig` (Mallwitz) und
  `i@zdvee.decrsfe` (ZEDF) verbrannten täglich einen Sendeslot und warfen bei
  Strato 521 „Domain does not exist". Bekannte + deutsche Geo-TLDs (de, com,
  ruhr, bayern, …) bleiben erlaubt. Commit `e68b512`.
